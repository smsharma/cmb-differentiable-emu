...
class tf_planck2018_lite:
    ...
    def __init__(
        ...
        if not (
            self.tt_emu_model.parameters
            == self.te_emu_model.parameters
            == self.ee_emu_model.parameters
        ):
        ...
        for i in range(self.nbinee):
            idxrepee = np.repeat(
                self.nbintt + self.nbinte + i, len(np.arange(beginee[i], endee[i]))
            )
            indicesee.append(
                self.plmax - 1 + self.plmax - 1 + np.arange(beginee[i], endee[i])
            )
        ...
        self.indices_rep = tf.concat(
            [self.indices_reptt, self.indices_repte, self.indices_repee], axis=0
        )
        self.indices = tf.concat(
            [self.indicestt, self.indiceste, self.indicesee], axis=0
        )
    ...
    def from_parameters_tensor_to_table(
        self,
        parameters_tensor,
    ):
        """
    ...
    def get_loglkl(self, parameters):
        ...
        cosmo_params = tf.transpose(
            parameters_table.lookup(tf.constant(self.tt_emu_model.parameters))
        )
        ...
        self.window_ttteee = tf.concat(
            [self.windowtt, self.windowte, self.windowee], axis=1
        )
    ...
    def loglkl(self, parameters, prodpri):
        ...
        loglike = tf.where(tf.equal(prodpri, 0.0), -0.5 * 2e12, loglike)
        return loglike
